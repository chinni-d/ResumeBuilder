...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function generatePreviewHTML(data) {
        ...
        if (data.full_name) {
            html += `<div class="name">${escapeHtml(data.full_name)}</div>`;
        }
        
        const contactInfo = [];
        if (data.email) contactInfo.push(escapeHtml(data.email));
        if (data.phone) contactInfo.push(escapeHtml(data.phone));
        if (data.address) contactInfo.push(escapeHtml(data.address));
        if (data.linkedin) contactInfo.push(escapeHtml(data.linkedin));
        if (data.github) contactInfo.push(escapeHtml(data.github));
